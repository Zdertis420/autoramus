...
// lines раздаёт номера координатных линий одной стрелки на одной диаграмме.
//
// Точки с одинаковой координатой лежат на одной прямой и делят номер. Мера
// снята с «теста»: у ветвящегося «контроля» вся магистраль несёт `yo=174` на
// семи секторах, а вертикаль входа — `xo=169` на трёх. Номер линии и есть то,
// чем Ramus сшивает сегменты в одну стрелку: раздай их порознь — и он покажет
// не стрелку, а стопку совпадающих обрывков, которые растащит первое же
// перетаскивание блока (FR-007 фичи 009).
//
// Разным стрелкам общая линия не выдаётся: в «тесте» координату 7.0 делят
// шесть разных стрелок, и номер у каждой свой.
type lines struct {
	counters *counters
