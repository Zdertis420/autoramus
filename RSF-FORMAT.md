...
`POINT_TYPE` в настоящих файлах чаще всего `-1` (76 точек из 101 в «тесте»);
значения `0` и `1` встречаются у точек на краю листа, но правила из данных не
видно. Генератор пишет `-1`.

Подпись и флаг ходят парой: где `SHOW_TEXT=0`, там вся рамка нулевая, и
наоборот. Ширина рамки растёт с длиной имени примерно как `4.3` на руну
(«вход2» 5 → 21, «контроль» 8 → 33, «Детали изделия» 14 → 58), высота при
шрифте `Dialog 10` кратна `9.80078125`.
