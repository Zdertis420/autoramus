...
func build(model *ir.Model, filename string, opts options, stdout, stderr io.Writer) int {
	...
	//
	// Модель сюда приходит уже разложенной: validate.Build раскладывает её
	// последней ступенью, чтобы проверить геометрию (specs/014). Вызов
	// оставлен — раскладка идемпотентна (TestApplyIsIdempotent), второй проход
	// ничего не меняет, а build() не обязан знать, откуда пришла модель.
	layout.Apply(model)
