...
func TestGolden(t *testing.T) {
	...
	for _, model := range models {
		t.Run(filepath.Base(model), func(t *testing.T) {
			...
			// Спрос — диагностика, а не непременно ошибка. Вид, который по
			// смыслу может быть только предупреждением, иначе не проверить:
			// coverage_test читает этот же каталог, а модель с одним лишь
			// duplicate_link ошибок не даёт и дать не должна — файл по ней
			// собирается, и стрелка в нём одна. Настоящая защита здесь в том,
			// что модель не молчит вовсе.
			if len(diags) == 0 {
				t.Fatalf("модель %s заведена ради диагностики, но не дала ни одной",
					filepath.Base(model))
			}
