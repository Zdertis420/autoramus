...
type Function struct {
	...
	// Tunnel — потоки, которые намеренно не переходят в декомпозицию этой
	// работы: конец стрелки на родительской диаграмме рисуется в скобках.
	// Связей не порождает — это утверждение о связи, уже объявленной в ICOM.
	//
	// Не путать с Endpoint.Tunnel: там туннель — часть геометрии, вычитанная
	// из чужого файла, здесь — утверждение автора о модели.
	Tunnel []Ref
